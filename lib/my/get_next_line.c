...
char *clear_text(char *buffer, char *auto_complete, char *result)
{
	...
	char *string = my_strdupcat(result, auto_complete);
...
char *check_valid_path(circular_dll_t *list, char *result, char *arg)
{
	...
	while (len != strlen(((complete_t *)data->data)->valid)) {
		...
		while (temp != list) {
			if (strlen(((complete_t *)temp->data)->valid) != 0 && strncmp(((complete_t *)data->data)->valid, ((complete_t *)temp->data)->valid, len) != 0) {
				len--;
...
{
	char *buffer = malloc(sizeof(char) * 3 + 1);
